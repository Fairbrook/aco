    ...
    def generate_solutions(self) -> None:
        for ant in self.ants:
            while (ant.position != self.end):
                ...
                edges.sort(key=self.__a)
                total_strength = functools.reduce(
                    lambda res, edge: res + self.__a(edge), edges, 0.0)
                ...
                for edge in edges:
                    ...
                    if r < p:
                        ...
                        ant.route.append(
                            (node.label, self.graph.points[edge.index].label))
                        # print(edge.distance)
    ...
    def pheromone_update(self):
        for point in self.graph.points:
            for edge in point.edges:
                ...
                edge.pheromones = (1-self.disipation) * \
                    edge.pheromones + accumulator
